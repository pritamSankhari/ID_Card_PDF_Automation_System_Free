...
$("#show_field_label_on").on('input',function(){
	show_field_label = 1;
})

$("#show_field_label_off").on('input',function(){
	show_field_label = 0;
})
	...
	for(let i=0;i<gridsForView.length;i++){
		...
		gridImage[i].addEventListener('change',function(){
			if(gridImage[i].value!='none'){
				...
				fields[i].imageWidth = gridsForView[i].children[0].style.width = gridImageWidth[i].value+"mm"
				fields[i].imageHeight = gridsForView[i].children[0].style.height = gridImageHeight[i].value+"mm"
		...
		gridImageWidth[i].addEventListener('change',function(){
			...
			if(fields[i].imageFlag){
				// FOR LAYOUT VIEW
				// ----------------------------------------------
				gridsForView[i].children[0].style.width = 'calc('+$("#points").val()+'*'+gridImageWidth[i].value+'mm)'
				// ----------------------------------------------
			}
			fields[i].imageWidth = gridImageWidth[i].value + "mm"
		...
		gridImageHeight[i].addEventListener('change',function(){
			...
			if(fields[i].imageFlag){
				// FOR LAYOUT VIEW
				// ----------------------------------------------
				gridsForView[i].children[0].style.height = 'calc('+$("#points").val()+'*'+gridImageHeight[i].value+'mm)'
				// ----------------------------------------------
			}
...
$('#btn-save').on('click',function(){
	...
	$.post(
		'do_save_layout.php',
		{
			...
			'show_field_label': show_field_label
		},
		function(data){
...
$('#points').on('change',function(){
	...
	for(let i=0;i<gridsForView.length;i++){
		...
		// gridsForView[i].style.left = 'calc('+fields[i].left+'*'+$('#points').val()+')'
		// gridsForView[i].style.top = 'calc('+fields[i].top+'*'+$('#points').val()+')'
